...
export class DepartmentServiceController {
    constructor(private departmentServiceService = new DepartmentServiceService()) { }

    public createService = async (req: Request, res: Response) => {
        try {
            const service = await this.departmentServiceService.createService(req.body);
            res.status(201).json({
    ...
    public getServiceById = async (req: Request, res: Response) => {
        try {
            const service = await this.departmentServiceService.getServiceById(Number(req.params.id));
            if (!service) {
                res.status(404).json({
    ...
    public deleteService = async (req: Request, res: Response) => {
        try {
            const result = await this.departmentServiceService.deleteService(Number(req.params.id));
            if (result === 0) {
                res.status(404).json({
                    message: "Service not found",
                    status: 404,
